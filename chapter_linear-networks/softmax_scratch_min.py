...
W_h = torch.normal(0, 0.01, size=(num_outputs, num_outputs), requires_grad=True)
b_h = torch.zeros(num_outputs, requires_grad=True)
...
#    hidden = torch.nn.ReLU()(torch.matmul(X.reshape(-1, W.shape[0]), W) + b)
